/** Paid DX analysis calibration; no PTY, and no changes to the existing four decision calibrations. */
...
const CASE_ID = 'plan-devex-peer-comparison-classification';
const describeCalibration = (body: () => void) => describeIfSelected('DX peer analysis semantic calibration', [CASE_ID], body);
...
describeCalibration(() => {
  testIfSelected(CASE_ID, async () => {
    ...
    // Three sequential JUDGE_MS calls fit the existing multi-capture budget;
    ...
    try {
      for (const calibration of devexPeerAnalysisCalibrations()) {
        ...
        expect(judgment.devexPeerComparison, calibration.name).toBeDefined();
        if (calibration.rejection) {
          expect(['missing', 'uncertain']).toContain(judgment.devexPeerComparison!.status);
          ...
          expect(judgment.devexPeerComparison!.status).toBe('complete');
          expect(result!.count).toBe(calibration.count);
          expect(result!.targetCallCount).toBe(4);
    ...
    try {
      collector?.addTest({ name: CASE_ID, suite: 'e2e-plan-devex-peer-comparison-classification', tier: 'e2e', passed: !failed,
